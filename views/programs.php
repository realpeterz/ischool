<!-- use the same id and class name except that "programs" would be replaced by your custom class name for your page -->
<div id="rightcolumn" class="grid_9 programs">

<h2>ACADEMIC PROGRAMS</h2> <!-- use h2 for all headings please -->

<p>The School of Information offers programs leading to the Master of Science in Information Studies degree, a Certificate of Advanced Study, a specialization in Preservation and Conservation Studies, a Learning Resources Certification for school librarians, and the Doctor of Philosophy degree. In its most recent ranking of LIS programs, U.S. News and World Report ranked the School of Information among the top ten LIS programs in the nation.</p> <!-- wrap any paragraph in p tags please -->
    <h2>Degree Programs</h2>

    <ul> <!-- ul>li>a for this kinda link lists -->
        <li><a href="http://www.ischool.utexas.edu/programs/phd.php">PhD - Doctor of Philosophy</a></li> 
        <li><a href="http://www.ischool.utexas.edu/programs/masters/">MSIS - Master of Science in Information Studies</a></li>
        <li><a href="http://www.ischool.utexas.edu/programs/minor.php">Minor - Undergraduate Minor</a></li>
        <!-- please put the absolute link in the href attribute in case we don't have time later to mess with one more level down the site. -->
    </ul>

<h2>Dual Degree Programs</h2>

    <ul>
        <li><a href="http://www.ischool.utexas.edu/programs/dual_degrees.php?expanddiv=women">MSIS/MA (Women's and Gender Studies)</a></li>
        <li><a href="http://www.ischool.utexas.edu/programs/dual_degrees.php?expanddiv=middle_east">MSIS/MA with Major in Middle Eastern Studies</a></li>
        <li><a href="http://www.ischool.utexas.edu/programs/dual_degrees.php?expanddiv=juris_doctor">MSIS/MA Juris Doctor Degree</a></li>
    </ul>

<h2>Specialist and Nondegree Programs</h2>

<p><a href="http://www.utexas.edu/student/registrar/catalogs/grad05-07/ch2/ch2.html#Nondegree.Student">Important Information Concerning Nondegree Students</a></p><!-- use p tags to wrap a stand-alone link like this so it won't be glued to the element before with a return -->

    <ul>
        <li><a href="http://www.ischool.utexas.edu/programs/specializations/cas.php">Certificate of Advanced Study</a></li>
        <li><a href="http://www.ischool.utexas.edu/programs/specializations/sslc.php">Standard School Librarian Certificate</a></li>
        <li><a href="http://www.utexas.edu/ogs/docport/">Graduate Portfolio Programs</a></li>
    </ul>

<h2>Capstone</h2>

    <ul>
        <li><a href="http://www.ischool.utexas.edu/programs/capstone/">Capstone Options Explained</a></li>
        <li><a href="http://www.ischool.utexas.edu/programs/capstone/institutions.php">Information for Institutions</a></li>
        <li><a href="http://www.ischool.utexas.edu/programs/capstone/pep_opportunities.php">Professional Experience Project (PEP) Opportunities</a></li>
    </ul>

 <p>Questions? Contact the School of Information at <a href="mailto:info@ischool.utexas.edu">info@ischool.utexas.edu</a></p><!-- an email address is not just an url. Don't forget to put "mailto:" -->

</div>
